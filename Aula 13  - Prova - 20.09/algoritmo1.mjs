...
const z = (y, fnComp) => {
    ...
    w = z(w, fnComp) // Chamando a função novamente e atribuindo o valor a variável "w" passando ele próprio como parâmetro
    v = z(v, fnComp) // Chamando a função novamente e atribuindo o valor a variável "v" passando ele próprio como parâmetro
...
const comparacao = (vetorDir, vetorEsq) => {
    if (vetorDir.razao_social === vetorEsq.razao_social) {
        return vetorDir.nome_motorista > vetorEsq.nome_motorista;
    }
    else return vetorDir.razao_social > vetorEsq.razao_social
}
