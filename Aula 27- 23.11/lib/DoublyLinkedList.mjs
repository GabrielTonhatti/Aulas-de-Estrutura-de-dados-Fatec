...
export default class DoublyLinkedList {
    ...
    remove(pos) {
        ...
        // 4º caso: remoção em posição intermediária
            ...
            // O nodo anterior passa a apontaratrás para o nodo anterior
            after.prev = before;
    ...
    indexOf(val) {
        ...
        let node1 = this.#head; // Começa no cabeça da lista
        let node2 = this.#tail; // Começa no cauda da lista

        for (let pos = 0; pos < middle; pos++) {
            // Verifica se o valor está no nodo1
            if (val === node1.data) return pos;

            // Verifica se o vaor está no nodo2
            if (val === node2.data) return this.#count - 1 - pos;

            // node1 avança bia next
            node1 = node1.next;
